...
**Current status (2026-06-13):** the platform is live on `https://www.growpoint.bg` (GitHub Pages) with a CloudFront test domain ready for the DNS cutover. Social login works via the Cognito hosted UI (**Google, LinkedIn, Apple**). The expert **package model Start / Grow / Spotlight** is live — **Start is the free default**; Grow/Spotlight are admin-grantable with Stripe checkout pending (Цецо). The June designer/tester change-request document is fully implemented (new copy, 7 области, package badges/ordering, 3-tier pricing card). Every push runs CI (secret scan + build + terraform validate).

### Changelog (June 2026, condensed)

- **2026-06-14 · paid-mentor model: invites, restrict, no approval:** corrected the pricing model — **all expert tiers are paid** (Start 9.99 € / Grow 29.99 / Spotlight 99.99 €/мес), clients stay free, and online payment (Stripe) is a "Очаквай скоро" placeholder. Because mentors are paid-only, **consultant approval was removed** entirely: a consultant is now public when their account is *active* (admin-comped/invited or — later — paid) and the profile passes the quality bar; the 11 existing profiles were grandfathered (`comped`). New mentor onboarding is **invite-only until Stripe**: an admin sends an email invite (`POST /admin/invites` → SES link `/auth?invite=TOKEN`) that the recipient redeems at bootstrap to get a free comped consultant account; self-serve consultant registration shows an invite-only notice. Admins can now **restrict (full-suspend) any account** (`PUT /admin/users/{id}/restrict`) — hides the public profile, blocks bookings, and disables Cognito sign-in (`AdminDisableUser`, reversible). The `/admin` page was reworked accordingly (invite form + pending list, restrict/unrestrict, public/featured/restricted filters; approve/reject removed).
- **2026-06-13 · project rename `careerdoc` → `growpoint` (infra + code), zero data loss / zero downtime:** the DynamoDB tables are now `growpoint-dev-{users,consultants,bookings}`, the S3 buckets `growpoint-dev-cv-*` / `growpoint-dev-frontend-*`, the Lambda `growpoint-dev-api`, plus IAM roles/policies, EventBridge rule, CloudFront function and resource tags. Because DynamoDB tables and S3 buckets can't be renamed in place, [scripts/migrate-tables-to-growpoint.cjs](scripts/migrate-tables-to-growpoint.cjs) creates the new tables mirroring the **exact** schema (key schema, all GSIs, PITR) and copies every item (counts verified 24/37/8), the two buckets are `aws s3 sync`'d, and Terraform is reconciled via `state rm` (orphan the old) + `import` (adopt the new) so no data resource is ever destroyed. **The Cognito user pool name is intentionally left unchanged** — a user pool's name is immutable in AWS, so renaming it would force a full pool replacement and wipe every account; it's an internal label only, so the pool id, all users (email + Google/Apple/LinkedIn), and the API Gateway URL are untouched. The old `careerdoc-dev-*` tables and buckets are kept intact as a rollback backup and are to be deleted once the cutover is confirmed. Verified live: API + 11 consultants on the new tables, GSI slug lookup, `smoke:prod` 14/14, IAM read+write on the new resources, CloudFront test domain, and a clean `terraform plan`.
- **2026-06-13 · admin monitoring → authoritative real numbers:** the `/admin` "Преглед на платформата" now reflects the real platform state, including the **Cognito user pool itself** rather than only the DynamoDB mirror. Backend `getCognitoUserStats` paginates `ListUsers` for **total / confirmed / unconfirmed / disabled / new-in-7-days / breakdown by login provider** (email vs Google/Apple/LinkedIn) and feeds `/admin/metrics`; also added **upcoming confirmed sessions** and **average review rating**. Infra: read-only `cognito-idp:ListUsers`/`DescribeUserPool` IAM scoped to the pool ARN + `USER_POOL_ID` Lambda env. UI: "Регистрирани акаунти" = Cognito (authoritative) with a confirmed split and a provider breakdown card; "Активирани профили" = the DynamoDB mirror (created on first authenticated request) with a **drift note** for registered-but-not-yet-activated accounts; new tiles for предстоящи сесии + среден рейтинг; and the old, misleading "visits aren't tracked" note is replaced (visits ARE counted via a cookieless first-party counter). Degrades gracefully to DB counts if Cognito is unreachable.
- **2026-06-13 · owner round (5 items):** (1) header bell notifications now open the **same full-text detail popup** as the `/notifications` page — extracted a shared `NotificationDetailModal` (portaled to `<body>`) used by both, and opening one marks just that one read so the badge clears; (2) confirmed the backend supports the three packets and made **Start the free default** — the pricing card shows „Винаги безплатно" for Start and „Очаквай скоро" for Grow/Spotlight (Stripe pending); (3) **package badges** — Grow keeps „Препоръчан експерт", Spotlight gets a fancier animated gold **„★ GrowPoint суперзвезди"** badge (honours `prefers-reduced-motion`); (4) `/auth` register heading „Създай профил" → **„Добре дошъл в GrowPoint"**; (5) **social-login onboarding popup polish** — the jank was the hidden avatar `<input type=file>` being forced visible by a global `input[type=file]{display:block}` rule; scoped it to `:not([hidden])` so the modal shows only the „Смени снимката" pill. Gates green; CORS temp-QA reverted & re-verified.
- **2026-06-12 · full audit + fixes:** professional audit (security code review of all new endpoints, live auth boundaries, CORS posture, both-role browser sweeps desktop/mobile/dark, edge cases) — two findings fixed: (1) consultant **owner-edit links never showed on the live public profile** (the check used `ownerUserId`, which the public payload strips — now compares against the viewer's own consultant profile); (2) a consultant with a **pending profile clicking „Виж публичната страница" got a raw English 404** — now a friendly owner-aware panel („Профилът ти още не е публичен" + CTA), with a Bulgarian not-available panel for everyone else. Notification UX rounds same week: full-text popup on `/notifications`, per-notification mark-as-read, booking actions open the sessions popup directly (`#sessions`), member card instant render + empty-name fix.
...
| --- | --- | --- | --- | --- |
| 2026-06-14 | Paid-mentor model (invites, restrict, no approval) | Pricing reverted to paid Start 9.99 (all tiers „Очаквай скоро"); approval removed — visibility now gated on active membership (`comped`/paid) + quality bar; 11 live profiles grandfathered (verified still public). New: email invites (SES link, redeemed at bootstrap → free comped account), restrict/full-suspend (Cognito `AdminDisableUser` + profile hidden, read-only IAM added), invite-only consultant signup notice. `/admin` reworked. Verified: `tsc`/`node --check`/`npm run build`/secret-scan PASS; terraform plan clean (no data/pool destroy); new endpoints 401 anon; consultants still 11; register gate DOM-verified | Passed | Owner: send a test invite + confirm the email; Стripe checkout to flip self-serve paid signup on |
| 2026-06-13 | Rename careerdoc → growpoint (infra + code) | New tables `growpoint-dev-{users,consultants,bookings}` (schema+GSIs+PITR mirrored, all items copied — counts 24/37/8 verified, GSI slug lookup OK), CV + frontend buckets synced, Lambda/IAM/tags → growpoint. Terraform reconciled (state rm old + import new); plan clean, **no DynamoDB destroy, Cognito pool untouched** (name pinned — pool id/users/API URL preserved). Live: API + 11 consultants on new tables, `smoke:prod` 14/14, IAM read+write confirmed, CloudFront test domain OK. Old careerdoc tables/buckets kept as backup | Passed; zero data loss / zero downtime | Owner: confirm cutover, then delete the old `careerdoc-dev-*` tables + buckets (rollback backup until then) |
| 2026-06-13 | Admin monitoring → authoritative | Account totals now read from the Cognito pool (`ListUsers`: total/confirmed/unconfirmed/disabled/by-provider/new-7d) + DB-vs-Cognito drift note; added upcoming confirmed sessions + avg review rating; replaced the false "visits not tracked" note. Read-only `cognito-idp:ListUsers`/`DescribeUserPool` IAM scoped to the pool ARN + `USER_POOL_ID` env (terraform apply, in-place). Verified: `tsc`/`node --check`/`npm run build`/secret scan PASS; metrics layout DOM-checked at 1280 & 390 (0 overflow, wide-chart span collapses on mobile); Lambda boots + `/admin/metrics` 401 anon + IAM/env confirmed | Passed (live Cognito read pending owner admin session) | Owner: open `/admin` to confirm the live Cognito numbers render |
| 2026-06-13 | Owner round (5 items) | Header bell → shared `NotificationDetailModal` popup (portaled, marks-one-read, badge clears); Start confirmed free in pricing card (Grow/Spotlight „Очаквай скоро"); Grow „Препоръчан експерт" + Spotlight animated gold „★ GrowPoint суперзвезди" badge (reduced-motion respected); `/auth` heading „Добре дошъл в GrowPoint"; onboarding popup fixed (hidden file-input root cause). DOM-verified live for all 5; `tsc`/`node --check`/`npm run build` (theme guard) + secret scan all pass; CORS temp-QA reverted & re-verified (localhost rejected, www allowed) | Passed; all 5 verified | Backlog unchanged (DNS cutover, Stripe → enables paid Grow/Spotlight, SES top) |
| 2026-06-12 | Full professional audit | Gates + drift clean; security review of new surface (package endpoint 401 anon, mark-read per-id own-record-only, public payload hides package internals — live-verified); both-role browser sweeps (client + consultant, desktop + 375px + dark): 0 overflow, 0 console errors, booking flow to submit, sessions-via-#sessions, files page both roles; 2 findings found & fixed (owner-edit links broken on live public profile; raw-English 404 for own pending profile) | Passed; fixes verified | Backlog unchanged (DNS cutover, Stripe, SES top) |
