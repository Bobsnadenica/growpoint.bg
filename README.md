...
This removes the GitHub Pages limitation where every new consultant route needs a committed static folder to avoid a refresh `404`. After Cloudflare DNS is switched to CloudFront, a newly approved consultant URL can load immediately through the SPA fallback.
...
Last QA pass: 2026-06-06 on `https://www.growpoint.bg` and the live AWS API.

This table is the current production-readiness map: what works, what still needs work, and what should be tested with real users next. A controlled live QA run on 2026-06-06 used disposable Cognito users, a disposable consultant profile, and a disposable booking; the test records were removed afterwards.

| Area | Status | What Works Now | Gaps / Risks | Next Step |
| --- | --- | --- | --- | --- |
| Public site shell | Works; CloudFront cutover prepared | Home, users, consultants, about, FAQ, contact, terms, privacy, auth, dashboard, admin, account, sitemap, robots, and manifest return `200` on `www.growpoint.bg`. S3 + CloudFront hosting is provisioned and deployed at `https://d30m6jtjij7col.cloudfront.net` with SPA fallback for unknown routes. | `https://growpoint.bg` without `www` fails TLS on the current GitHub Pages path. Cloudflare DNS is still pointed at GitHub Pages. | Validate the ACM DNS records in Cloudflare, attach the issued certificate to CloudFront aliases, then point `www` and apex to CloudFront. |
...
| Image handling | Works | Uploaded public profile images render from signed S3 URLs, and real profile avatars can open in the full-screen lightbox. SEO metadata falls back to a stable GrowPoint image when profile media is signed/temporary. | Cached social previews will use the fallback image until stable public image derivatives exist. | Generate stable public profile image derivatives for approved profiles. |
...
| Booking requests | Verified live | Automated live QA creates a disposable booking, confirms it as the consultant, downloads ICS, and verifies in-app notifications. CORS preflight from `www.growpoint.bg` and CloudFront works. | Decline, cancel, and reschedule were not exercised in the latest automated lifecycle run. | Extend `scripts/smoke-production.mjs` to cover decline, cancel, and reschedule paths. |
...
| Emails | Partly verified live | Cognito resend confirmation and forgot-password requests returned email delivery details. GrowPoint-branded backend email templates exist for booking/admin flows. | Actual inbox contents were not inspected. SES sender identity must be verified before switching Cognito to `contactus@growpoint.bg`. | Test received verification, reset, booking, admin message, and reminder emails with a controlled inbox. |
| Scaling and reliability | MVP-ready, not scale-complete | DynamoDB pay-per-request tables, point-in-time recovery, Lambda/API Gateway throttling, dynamic multi-origin CORS, private S3 frontend hosting, and CloudFront SPA fallback are configured. Public routes and API health are live. | Public search/listing is still simple and should not be treated as a large-scale search engine. Observability, alarms, rate limits, and structured validation should be expanded. | Add CloudWatch alarms, structured validation tests, pagination/search indexes, and load testing before a larger campaign. |
...
- `node scripts/smoke-production.mjs --cloudfront` passed 15/15 checks, including unknown consultant route SPA fallback.
- `npm run smoke:prod` passed 14/14 checks against `https://www.growpoint.bg`.
- `npm run smoke:prod:live` passed 24/24 checks with disposable production users, consultant, booking, messages, ICS, session confirmation, review, notification mark-read, and cleanup.
- Independent cleanup verification found no remaining `codex-` Cognito users, no `codex-` DynamoDB users, and no disposable `Codex QA` consultant rows.
...
| 2026-06-06 | CloudFront/S3 hosting | Terraform apply, `npm run deploy:cloudfront`, `node scripts/smoke-production.mjs --cloudfront` | Passed on CloudFront test domain | Complete Cloudflare certificate validation and DNS cutover. |
| 2026-06-06 | Automated live smoke | `npm run smoke:prod:live`, latest run `l32svnlo44g` | Passed 24/24 and cleanup verified | Extend script for admin credentials, documents, media, decline/cancel/reschedule. |
...
| --- | --- | --- | --- |
| P0 | CloudFront DNS cutover | Add ACM validation records in Cloudflare, attach the issued certificate to CloudFront aliases, then point `www` and apex to CloudFront | Users may type the apex domain directly, and CloudFront is needed for live SPA fallback. |
...
| --- | --- | --- |
| P0 | Finish CloudFront custom-domain cutover | AWS S3/CloudFront hosting is prepared, but Cloudflare DNS validation/cutover is still required. |
| P1 | Add CloudWatch alarms and API Gateway throttling | MVP infrastructure works, but production monitoring is thin. |
| P1 | Expand repeatable production smoke scripts | Core public, CloudFront, and live lifecycle smoke scripts exist; admin, documents, media, decline/cancel/reschedule, and mobile checks still need coverage. |
...
4. after Cloudflare DNS points to CloudFront, run `npm run smoke:prod`

For the custom domain cutover:

1. read `frontend_certificate_validation_records` from Terraform output
2. create those CNAME records in Cloudflare
...
6. update Cloudflare DNS to point apex and `www` to the CloudFront distribution
