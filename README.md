...
- **Roles & groups:** added a `consultants` Cognito group; `/auth/bootstrap` now treats group membership as authoritative (group → `role=consultant` + consultant draft), so a Cognito-created user can be designated a mentor without re-registering. `admin` remains a separate group.
...
- Roles: client vs consultant is normally chosen at registration, but the `consultants` Cognito group is authoritative — add a user to it (`aws cognito-idp admin-add-user-to-group --user-pool-id eu-west-1_Yl8EDZKFA --username <email> --group-name consultants`) and on their next login `/auth/bootstrap` forces `role=consultant` and creates a consultant draft. This is how a Cognito-created user is designated a mentor.
