...
- `node scripts/smoke-production.mjs --cloudfront` passed 15/15 checks on run `830xasa9ac`, including unknown consultant route SPA fallback.
- `npm run smoke:prod` passed 14/14 checks on run `o7pwt9rpl5n` against `https://www.growpoint.bg`.
...
- SES domain verification resources were created for `growpoint.bg`; Cloudflare DNS records and SES production-access approval are still pending before GrowPoint-branded transactional emails can be production-ready.
...
| 2026-06-07 | SES readiness | SES account and identities inspected; Terraform SES domain identity/DKIM resources added | Blocked by DNS and SES production access | Add Cloudflare records, request SES production access, then test inbox delivery and copy. |
...
| P0 | Finish GrowPoint SES sender setup | Terraform can output SES domain verification/DKIM records, but Cloudflare DNS and SES production access are still required before backend emails can send reliably from `contactus@growpoint.bg`. |
| P1 | Add CloudWatch alarms and API Gateway throttling | MVP infrastructure works, but production monitoring is thin. |
| P1 | Expand repeatable production smoke scripts | Core public, CloudFront, media, documents, booking lifecycle, messages, reviews, and cleanup are covered; admin browser flows, mobile visual checks, email inbox checks, reminders, and payments still need coverage. |
...
When `ses_domain_identity = "growpoint.bg"` is set, Terraform creates the SES identity and outputs the DNS records Cloudflare must add:
...
3. create `ses_domain_verification_record` and `ses_domain_dkim_records` in Cloudflare
...
- GrowPoint-branded transactional email is blocked until Cloudflare SES DNS records and AWS SES production access are complete
