...
Last QA pass: 2026-06-06 on `https://www.growpoint.bg`.

This table is the current production-readiness map: what works, what still needs work, and what should be tested with real users next. Mutating flows such as account creation, password reset, booking creation, sending messages, confirming sessions, deleting profiles, and submitting reviews should be tested only with controlled test users and a test booking, because they send emails or write production data.

| Area | Status | What Works Now | Gaps / Risks | Next Step |
| --- | --- | --- | --- | --- |
| Public site shell | Works | Home, users, consultants, about, FAQ, contact, terms, privacy, auth, dashboard, admin, account, sitemap, robots, and manifest return `200` on `www.growpoint.bg`. The beta ribbon, cookie banner, white/default theme, dark theme toggle, and route metadata render. | `https://growpoint.bg` without `www` fails TLS because the certificate does not match the apex domain. | Fix GitHub Pages/DNS certificate coverage for the apex domain and redirect apex to `www`. |
| SEO | Works with manual route list | Static route files, canonical links, Open Graph/Twitter tags, structured data, `robots.txt`, and `sitemap.xml` are generated by `scripts/site-build.mjs`. The Dimitar Cyrillic profile route is now included in the sitemap. | New user-created public profiles need static route entries added to `src/lib/seo-data.json` if they must return server-level `200` from GitHub Pages. | Automate public-profile static route generation from approved consultants or move public profiles to a host that can serve dynamic SPA fallbacks with `200`. |
| Public consultant directory | Works | Live API `/consultants` returns approved public profiles, including the approved Dimitar profile. Directory search/filter routes render and query state is reflected in the URL. | Some profile content is still test-quality. The Dimitar profile currently contains placeholder `Test...` copy, no availability, and price is shown as inquiry-only. | Clean real profile content before promotion; add admin quality checklist for public launch. |
| Public consultant profile | Works | Public profile pages fetch live data, show uploaded avatar/cover media, share link, and no longer show the old `Consultant profile not found` for the approved Cyrillic slug. The `Бърз профил` block/label has been removed from profile surfaces in the current source/build. | Booking CTA is only useful when a profile has valid future availability. Static `200` routes still depend on generated route files. | Require availability/price/profile quality before publishing, or show clearer "request availability" flow. |
| Image handling | Works | Uploaded public profile images render from signed S3 URLs, and real profile avatars can open in the full-screen lightbox. | Signed media URLs expire, so cached social previews may not always keep profile media. | Use stable public image derivatives for approved public profile previews. |
| Auth: login/register/verification | Partly verified | Auth page renders login/register tabs. Cognito admin login works through the configured app client. Confirmation-code resend is implemented through Cognito resend, not second signup. | New account registration, email verification delivery, resend-code UX, forgot-password email, reset confirmation, and new-password-required challenge need controlled inbox testing in production. | Run an auth test script with a disposable GrowPoint inbox and record expected email subjects/copy. |
| Admin panel | Works read-only | Admin Cognito user can call protected admin APIs. `/admin/consultants` returns `200` and includes 17 records; admin detail for Dimitar returns `200`, `approved`. Browser admin panel has previously loaded without the old profile-not-found dashboard error. | Admin messaging, approve/reject/delete actions intentionally were not exercised during this pass because they mutate real data. | Create a disposable consultant profile and test approve, reject, preview, message, and deletion end to end. |
| Consultant profile editing | Partly verified | Backend ownership/canonical-profile logic is in place so consultant updates should map to the admin-visible canonical profile. Media upload/save paths are implemented. | Needs a controlled consultant account retest after every major dashboard change. | Use a test consultant to update text, price, type, city, availability, avatar, and cover; verify admin/public views match. |
| Booking requests | Partly verified | Public booking endpoint exists; booking status lifecycle is implemented. CORS preflight from `www.growpoint.bg` works. | Creating a booking was not executed in this pass to avoid production writes. Concurrency protection should continue to be monitored. | Create a test client + test consultant slot and verify request, email, notification, accept/decline, cancel, and calendar behavior. |
| Booking messages | Route fixed, needs live scenario test | API Gateway now exposes `GET/POST /bookings/{bookingId}/messages`; unauthenticated requests return `401`, proving the route reaches JWT protection instead of missing-route failure. | Actual message sending was not executed because it would write to a production booking. | Test with a confirmed test booking; verify both parties see messages and top-bar unread counts update. |
| Session confirmation | Route fixed, needs live scenario test | API Gateway now exposes `POST /bookings/{bookingId}/session-confirm`; unauthenticated requests return `401`. | Actual confirmation was not executed because it changes booking state. | Test after a past confirmed test session; verify both-party confirmation unlocks review flow. |
| Notifications | Works, UX should improve | Backend notifications exist and top-bar notification/message indicators are implemented for logged-in non-admin users. Dashboard notification history exists. | Notifications should not feel like only a dashboard section. A social-style top-bar popover would be more natural, similar to LinkedIn. | Replace or supplement the dashboard section with a notification popover: unread list, message list, mark-read action, deep links to booking/session, mobile sheet version. |
...
| Reviews | Implemented, needs live scenario test | Reviews require confirmed booking, session time in the past, both sides confirming the session happened, and a 60-day review window. | Needs real test booking lifecycle data to verify the UI unlocks exactly when expected. | Create/past-date a test session, confirm both sides, submit review, verify public rating and recent review display. |
| Emails | Implemented, delivery depends on config | GrowPoint-branded templates exist for platform emails; Cognito verification uses Cognito delivery unless SES sender is configured. | Email delivery/copy was not retested in this pass. SES sender identity must be verified before switching Cognito to `contactus@growpoint.bg`. | Test registration, verification, resend code, forgot password, booking, admin message, and reminder emails with a controlled inbox. |
| Scaling and reliability | MVP-ready, not scale-complete | DynamoDB pay-per-request tables, point-in-time recovery, Lambda/API Gateway, and CORS are configured. Public routes and API health are live. | Public search/listing is still simple and should not be treated as a large-scale search engine. Observability, alarms, rate limits, and structured validation should be expanded. | Add CloudWatch alarms, API throttling, structured validation tests, pagination/search indexes, and load testing before a larger campaign. |
| Payments/subscriptions | Not production-ready | UI can present plans/membership concepts. | Real payments/subscriptions are not wired end to end. | Decide payment provider and implement a proper checkout/billing flow before charging users. |

Latest smoke evidence from 2026-06-06:

- `www.growpoint.bg` public routes listed above returned `200`.
- API `/health` returned `{"ok":true,"service":"growpoint-api"}`.
- API `/consultants` includes the approved Dimitar profile.
